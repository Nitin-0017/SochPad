...
import { Trophy, Zap, Wind, Ghost, Eye, Clock, BrainCircuit, Lightbulb, Target } from 'lucide-react';
...
export default function Analytics({ tasks, completions, apiKey }) {
  ...
  const getArchetype = () => {
    if (procrastinationScore < 20) return { label:'Silent Achiever', icon: Trophy, desc:'You just do it. Quietly dominant.' };
    if (procrastinationScore < 50) return { label:'Last-Minute Legend', icon: Zap, desc:'You thrive under pressure. Chaotic but effective.' };
    if (procrastinationScore < 80) return { label:'Certified Overthinker', icon: Wind, desc:"You've thought about doing it 10 times. That counts, right?" };
    return { label:'The Ghost', icon: Ghost, desc:'Your tasks have sent search parties. Please respond.' };
  };
  ...
  const fetchAnalysis = async () => {
    ...
    setLoading(true);
    try {
      const res = await getWeeklyAnalysis(tasks, completions, apiKey);
      setAnalysis(res);
    } catch {}
    setLoading(false);
  };
    ...
    <div style={{ padding:'0 0 40px' }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:16, marginBottom:24 }}>
        {[
          ...
          { label:'Done (7 days)', value:last7.length, sub:'tasks completed', color:'#2A7D63' },
          { label:'Done (30 days)', value:last30.length, sub:'tasks completed', color:'#7B5EA7' },
        ].map(s => (
      ...
      <div className="card" style={{ marginBottom:20 }}>
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:12 }}>
          ...
          <span style={{ fontSize:20, display:'flex' }}><archetype.icon size={24} color="var(--accent)" /></span>
        </div>
              ...
              return (
                <div key={cat}>
                  <div style={{ display:'flex', justifyContent:'space-between', marginBottom:5 }}>
                    <span style={{ fontSize:13, fontWeight:600 }}>{getCategoryEmoji(cat)} {cat}</span>
            ...
            {mostAvoided.map(t => (
              <div key={t.id} style={{
                ...
                <div>
                  ...
                  <p style={{ fontSize:11, color:'var(--text-light)' }}>{t.category}</p>
                </div>
                <span style={{
